...
import matplotlib
matplotlib.use("Agg")  # chạy được khi không có display
...
def load_and_preprocess_data(file_path: str,
    ...
    """Đọc JSONL lớn, gộp trùng (user,item), lọc theo min_ratings; trả về (df_train, df_test) hoặc (df_filtered, None)."""
...
# Model / Grid
# -----------------------------
def grid_search_user(data: Dataset, fast: bool = True) -> Dict[str, Any]:
    """GridSearch nhỏ gọn tiết kiệm tài nguyên."""
    if fast:
        param_grid = {
            'k': [30],
            'min_k': [5],
        ...
        param_grid = {
            'k': [20, 30, 50],
            'min_k': [3, 5],
            'sim_options': {'name': ['msd', 'cosine', 'pearson_baseline'], 'user_based': [True]}
        }
...
# Train/Eval
# -----------------------------
def run_user_based(df_train: pd.DataFrame, df_test: pd.DataFrame, random_state: int,
                   do_grid: bool, fig_dir: str):
    reader = Reader(rating_scale=(1, 5))

    if df_test is None:
        data = Dataset.load_from_df(df_train[['user_id', 'item_id', 'rating']], reader)
        best_params = grid_search_user(data, fast=True) if do_grid else \
            {'k': 30, 'min_k': 5, 'sim_options': {'name': 'msd', 'user_based': True}}
        ...
        best_params = grid_search_user(data_train, fast=True) if do_grid else \
            {'k': 30, 'min_k': 5, 'sim_options': {'name': 'msd', 'user_based': True}}
    ...
    plot_rmse_mae(r, m, "User-Based CF", train_time, infer_time, best_params, fig_dir)
...
def main():
    ...
    ap.add_argument('--fig_dir', type=str, default='figs_user_based')
    args = ap.parse_args()
    ...
    run_user_based(df_train, df_test, random_state=args.seed,
